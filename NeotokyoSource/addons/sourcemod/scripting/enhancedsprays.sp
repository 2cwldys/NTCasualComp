...
{
	CreateConVar("sm_enhancedsprays_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);
	...
	g_hCvarDistance = CreateConVar("sm_enhancedsprays_distance", "115", "Distance players can apply sprays to walls", _, true, 0.0);
	g_fDistance = GetConVarFloat(g_hCvarDistance);
	HookConVarChange(g_hCvarDistance, OnConVarChange);

	g_hCvarDelay = FindConVar("decalfrequency");
...
public Action:OnPlayerRunCmd(iClient, &iButtons, &iImpulse, Float:fVelocity[3], Float:fAngles[3], &iWeapon)
{
	...
	if(!IsPlayerAlive(iClient) && !CheckCommandAccess(iClient, "sm_enhancedsprays_deadflag", 0))
